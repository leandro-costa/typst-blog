...
- [x] 1.3 Documentar no design.md a assinatura confirmada e o fallback (ex.: exibir via `BufferedImage`)

## 2. Implementação da mágica

> Implementada e validada em `posts/aulas/plantuml-demo.ipynb` (células `maven-dep` e `magic-impl`, executadas com outputs `image/png` salvos).

- [x] 2.1 Escrever a célula de configuração com `%maven net.sourceforge.plantuml:plantuml:<versão pinada>`
- [x] 2.2 Importar as classes necessárias (`SourceStringReader`, `ByteArrayOutputStream`, `Base64`, API do IJava)
- [x] 2.3 Registrar a mágica `plantuml` via `getMagics().registerCellMagic(...)`
- [x] 2.4 Implementar autocompletar das tags `@startuml`/`@enduml` quando ausentes
- [x] 2.5 Gerar bytes PNG com `SourceStringReader.outputImage` e codificar em base64
- [x] 2.6 Emitir a saída como `image/png` (display_data com mimetype `image/png`), não `text/html`
- [x] 2.7 Tratar exceções e retornar mensagem de erro legível
...
- [x] 4.5 Testar comportamento de erro para diagrama inválido
